...
Proteases are unchanged from Model 5 (PMs + FP2/3, logistic `[E]`).

---

## What changed vs Model 5 (and why)

**Problem in Models 4–5:** Hz forms from the **whole** lipid-associated Fe³⁺ pool at literature `k_hz`. That drains assay free haem (`aq + lip`) below Combrink basal Hm even when digestion looks better.

**Idea:** only a fraction of lipid Fe is crystal-competent:

```text
Fe(III)_aq  ⇄  Fe(III)_lip  ⇄  Fe(III)_xtal  →(k_hz)→  Hz
```

with `K_xtal = [Fe]_xtal / [Fe]_lip ≈ 0.08` so most non-Hz Fe stays in aq+lip.

| Item | Model 5 | Model 6 |
|------|---------|---------|
| Proteases | PMs + FP2/3; logistic | **Same** |
| Fe(III) | aq ⇄ lip → Hz | aq ⇄ lip ⇄ **xtal** → Hz |
| `v_hz` substrate | `[Fe(III)]_lip` | `[Fe(III)]_xtal` |
| Plotted free haem | aq + lip | aq + lip (xtal excluded) |
...
flowchart LR
  ...
  HbDV -->|"PMs + FP2 + FP3"| Fe2["conc_fe2pp"]
...
| `conc_fe3pp_xtal` | Crystal-competent Fe(III) |
| `conc_hz` | Haemozoin |
| `conc_hb_rbc` | Remaining host Hb |

Init: `[Hb_DV, Fe2, Fe3_aq, Fe3_lip, Fe3_xtal, Hz]`.

---

## Governing equations

Uptake / digestion / oxidation (logistic enzyme clock; PMs + FP2/3):
...
Lipid / crystal partition and exchange:
...
v_aq↔lip = k_lipid_ex · ( [Fe(III)]_aq − [Fe(III)]_lip / K_eff )

# Lipid ⇄ crystal-competent Fe(III) exchange
v_lip↔xtal = k_xtal_ex · ( [Fe(III)]_lip − [Fe(III)]_xtal / K_xtal )

# Haemozoin formation (from crystal-competent pool)
v_hz = k_hz · [Fe(III)]_xtal
```
...
# Aqueous Fe(III)PPIX
d[Fe(III)]_aq / dt   = v_ox − v_red − v_aq↔lip

# Lipid-associated Fe(III)PPIX
d[Fe(III)]_lip / dt  = v_aq↔lip − v_lip↔xtal

# Crystal-competent Fe(III)PPIX
d[Fe(III)]_xtal / dt = v_lip↔xtal − v_hz
...
| `k_xtal_ex` | 5 | min⁻¹ | Rate constant for lipid ⇄ crystal-competent Fe(III) exchange |
| `K_xtal` | 0.08 | — | Equilibrium ratio `[Fe(III)]_xtal / [Fe(III)]_lip` |
| `k_hz` | 0.12 | min⁻¹ | First-order rate constant for haemozoin formation from crystal-competent Fe(III) |
...
## Behaviour vs Model 5

- Higher standing free haem (aq+lip); slightly lower end Hz for the same digestion flux.
