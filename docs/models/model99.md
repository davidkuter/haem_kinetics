...
**Code:** [`haem_kinetics/models/model99.py`](../../haem_kinetics/models/model99.py)  
**Up:** [Model index](../models.md) · **Prev:** [Model 9](model9.md)

**This is not a mechanistic ladder step.** Same ODEs as Model 9a (including crystal-area growth, sphere 2/3). Two knobs are freed against Garnie Dd2 to ask: *if* inner-vesicle lysis were slower and *if* the interfacial fraction were slightly larger, could Hb and Hm approach the assay?

They are **not** Klemba's `t½ < 20 min` and **not** `K_xtal = 3δ/R`. **`f_exp` is unchanged** (Model 2b two-phase): do not retune `a, b` here. The `k_release` / `K_xtal` numbers below were chosen on the **Model 2a** inventory and are not refit. Model 8 keeps the citations; Model 9a keeps the area law. This page is a diagnostic.

**Model 99 is a permanent what-if slot** — new mechanistic models are numbered below 99 without requiring rename.

---

## What changed vs Model 9a

| Item | Model 9a | Model 99 |
|------|----------|----------|
...
So `k_99(t) = 0.4774 · k_Klemba · s_PM(t)`. The scale was not refit after Model 6 or Model 9.
...
`k_release` and `K_xtal` were least-squares / grid chosen on the interfacial model **with Model 2a uptake**. Later models inherit 2b, `k_release ∝ s_PM`, and Model 9 area growth. Late `v_up` is larger, so standing cargo overshoots Dd2 Hb. **Do not refit these two knobs** to recover the old χ² — that would be a second Garnie fit, not chemistry.
...
**Vs Model 9a:** the 2a-tuned knobs overshoot Hb (χ²_red 0.74 → 52.7, signed +2.26). Hm χ²_red 53 → 1.29 because this page inherits the area law and a larger `K_xtal`; that is not a reason to treat 0.10 as chemistry. Hz RMSE 2.30 → 2.82. DV Fe unchanged. Do not refit `k_release` / `K_xtal` here.
...
- Klemba bound (what Model 99 **violates** at plateau): [model5.md](model5.md)
- Model 6 `s_PM` lysis clock (what Model 99 **scales**): [model6.md](model6.md)
- Geometric `K_xtal` (what Model 99 **replaces**): [model8.md](model8.md)
- Crystal-area growth (what Model 99 **inherits**): [model9.md](model9.md)
- Model 2b `f_exp` (what Model 99 **does not retune**): [model2.md](model2.md)
