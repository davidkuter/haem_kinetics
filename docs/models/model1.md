...
**What that implies vs experiment (Dd2):** DV Hb collapses almost immediately (protease capacity ≫ uptake), Hz forms from the bulk Fe³⁺ pool, and there is **no mechanism for a standing basal free-haem (Hm) pool** of the size Combrink reports.
...
- Enzyme capacity ≫ uptake → DV Hb collapses immediately. That is a **mechanistic** mismatch (constant full PaxDB `[E]` from `t` = 0), not something to patch in the RHS; later models change uptake/enzyme timing for biological reasons.
- Free Fe³⁺ is drained by `v_hz = k_hz · [Fe(III)]` with no non-crystallizing reservoir → simulated free haem undershoots Combrink basal Hm.
