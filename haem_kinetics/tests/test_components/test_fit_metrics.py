...
def test_model2a_hz_beats_model1():
...
def test_model2b_dv_fe_beats_model2a():
    """Two-phase f_exp (still × remaining host) should improve Dd2 inventory vs one phase."""
    kwargs = dict(t=[0, 1700], init=[0.018, 0.0, 0.0, 0.36], t_eval=range(0, 1700, 20))
    m2 = Model2a()
    m2.run(**kwargs)
    m2.score_vs_experiment()
    m2b = Model2b()
    m2b.run(**kwargs)
    m2b.score_vs_experiment()
    assert m2b.fit_metrics['DV_Fe']['rmse'] < m2.fit_metrics['DV_Fe']['rmse']
    assert m2b.fit_metrics['Hz']['rmse'] < m2.fit_metrics['Hz']['rmse']


def test_dv_fe_skips_hb_assay_and_free_haem_lumped_cols():
    """Assay Hb and aq+lip+xtal Hm lumps must not double-count in DV Fe."""
    idx = [20.0, 24.0]
    df = pd.DataFrame({
        'conc_hb_htv': [1.0, 1.0],
        'conc_hb_dv': [0.0, 0.0],
        ...
        'conc_fe2pp': [0.0, 0.0],
        'conc_fe3pp_aq': [0.4, 0.4],
        'conc_fe3pp_lip': [0.4, 0.4],
        'conc_fe3pp_xtal': [0.2, 0.2],
