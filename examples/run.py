...
for plot_name, cls, init, extra in runs:
    ...
    dv_cols = [
        c
        for c in model.concentrations.columns
        if c.startswith("conc_") and c != "conc_hb_rbc" and c not in ("conc_hb_dv_obs",)
    ]
    tot = float(model.concentrations[dv_cols].iloc[-1].sum() + host.iloc[-1])
    hz = (
        float(model.concentrations["conc_hz"].iloc[-1])
        if "conc_hz" in model.concentrations
        else 0.0
    )
    print(
        f"  end total Fe={tot:.2f} fg  Hz={hz:.2f} fg  host={float(host.iloc[-1]):.2f} fg"
    )
